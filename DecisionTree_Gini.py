...
def ID3(S, attributes, label):
    ...
    if S[6].nunique() == 1:
        ...
        best_attribute = train_using_gini(S, attributes)
...
def train_using_gini(S, attributes):
    # if attributes is None or len(attributes) == 0:
    #     return 0
    # print('--in entropy--')
    ...
    total_entropy = 1 - pow((len(S.loc[S[6] == 'unacc'])/total), 2) - pow((len(S.loc[S[6] == 'acc'])/total), 2)\
                      - pow((len(S.loc[S[6] == 'good'])/total), 2) - pow((len(S.loc[S[6] == 'vgood'])/total), 2)
    # print(total_entropy)
    ...
    for a in attributes:
        ...
        for t in att_things:
            ...
            if len(S_t) > 0:
                expected_gini += (len(S_t)/total) * (1 - pow((len(S_t.loc[S_t[6] == 'unacc'])/len(S_t)), 2)
                                                          - pow((len(S_t.loc[S_t[6] == 'acc'])/len(S_t)), 2)
                                                          - pow((len(S_t.loc[S_t[6] == 'good'])/len(S_t)), 2)
                                                          - pow((len(S_t.loc[S_t[6] == 'vgood'])/len(S_t)), 2))

        IG = total_entropy - expected_gini
        # print(IG)
        if IG > best_IG:
            best_attribute = a
            best_IG = IG
