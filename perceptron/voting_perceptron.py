...
def main():
    ...
    for index, row in test_data.iterrows():
        prediction = 0
        for x in range(0, len(all_vectors)):
            prediction += all_vectors[x][1] * np.sign(1*all_vectors[x][0][0] + row[0] * all_vectors[x][0][1] + row[1] * all_vectors[x][0][2] + row[2] * all_vectors[x][0][3] + row[3] * all_vectors[x][0][4])

        total += 1
        if np.sign(prediction) != 2 * row[4] - 1:
            mistakes += 1
